...
```{r}
pred_logistic_model <- predict(logistic_model_train, type="response", newdata =birth_train_data )
```


```{r}
prediction_and_labels_df <- data.frame(pred_logistic_model, birth_train_data$is_preterm_binary)
...
```{r message=FALSE}
auroc_value <- auc(prediction_and_labels_clean$labels, prediction_and_labels_clean$predictions)
auroc_value
```
